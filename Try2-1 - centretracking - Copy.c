...
float P=2; //to eliminate any zero bias value present in the error
float S=96; //set velocity
float lm=0,rm=0;
...
/////////////////////////////MOVING THE BOT/////////////////////////////
	forward();
	if(PID>-60 && PID<60){lm=S-PID;rm=S+PID;}
	else if(PID>60 && PID<120){lm=0;rm=S+PID;}
	else if(PID>-60 && PID<-120){lm=S-PID;rm=0;}
	else if(PID>120){left();lm=S+PID;rm=S+PID;}
	else if(PID<-120){right();lm=S-PID;rm=S-PID;}
